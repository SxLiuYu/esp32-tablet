 ...
 *
 * Final API (verified against esp-protocols esp_websocket_client master, 2026-06-14):
 *   - esp_websocket_client_send_text(client, data, len, timeout)  -- 4 args
 *   - event->data_ptr / event->data_len
 *   - cfg.network_timeout_ms (not timeout_ms)
 */
#include "ws_client.h"
#include "ui.h"
#include "rgb.h"
...
static TaskHandle_t s_audio_task_h = NULL;
...
{
    if (!s_audio_active || !s_client || !esp_websocket_client_is_connected(s_client)) return;
    char msg[8192];
    ...
    case WEBSOCKET_EVENT_DATA:
        if (event && event->data_ptr && event->data_len > 0) {
            ...
            if (len < 16384 - s_resp_len) {
                memcpy(s_resp_buf + s_resp_len, event->data_ptr, len);
                s_resp_len += len;
